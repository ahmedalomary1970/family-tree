<script>
  ...
  const WORKSPACE_CM = 150;
  const WORKSPACE_MM = WORKSPACE_CM * 18; // 1500mm
...
// ===============================
// طباعة احترافية: اختيار حجم الورق (A4/A3/A2/A1) بالـ mm
...
function resolveAutoParentChildPair(idA, idB) {
  const a = getPersonById(idA);
  const b = getPersonById(idB);
  if (!a || !b || a.id === b.id) return null;

  const ringA = ringIndexById(a.ringId);
  const ringB = ringIndexById(b.ringId);

  // الجيل الأعلى هو الرقم الأصغر: 3 أب بالنسبة إلى 4، و4 أب بالنسبة إلى 5، وهكذا.
  // لذلك يكون اتجاه الرابط دائماً من الجيل الأعلى إلى الجيل الأدنى.
  // وإذا كان الشخصان في نفس الجيل نمتنع عن الربط التلقائي حتى لا ننشئ علاقة خاطئة.
  if (ringA === ringB) return null;

  if (ringA < ringB) {
    return { parentId: a.id, childId: b.id };
  }
  return { parentId: b.id, childId: a.id };
}

function ensureAutoLinkForSelectedPair() {
  if (linkMode) return;
  if (!Array.isArray(selectedPersonIds) || selectedPersonIds.length !== 2) return;

  const pair = resolveAutoParentChildPair(selectedPersonIds[0], selectedPersonIds[1]);
  if (!pair) return;

  const forward = links.find((l) => l.parentId === pair.parentId && l.childId === pair.childId) || null;
  if (forward) {
    selectedLinkKey = mkLinkKey(forward);
    toast("الرابط موجود مسبقاً");
    return;
  }

  const reverse = links.find((l) => l.parentId === pair.childId && l.childId === pair.parentId) || null;
  if (reverse) {
    links = links.map((l) =>
      l === reverse
        ? {
            ...
            bends: Array.isArray(l.bends) ? l.bends : []
          }
        : l
    );
    selectedLinkKey = `${pair.parentId}|${pair.childId}`;
    toast("تم تصحيح اتجاه الرابط");
    try { saveNow(); } catch (e) {}
    return;
  }

  links = [...links, { parentId: pair.parentId, childId: pair.childId, bends: [] }];
  selectedLinkKey = `${pair.parentId}|${pair.childId}`;
  toast("تم إنشاء الرابط تلقائياً");
  try { saveNow(); } catch (e) {}
}
...
function resetSelectedLinkBendsFromPanel() {
  ...
  function onPersonPointerDown(e, p) {
    ...
    if (isMultiToggle) {
      ...
      if (selectedPersonIds.length === 2) ensureAutoLinkForSelectedPair();
      return;
    }
...
async function createShareLink() {
  if (!authUser || !cloudProjectId) {
    cloudStatus = "احفظ المشروع في السحابة أولاً";
    return;
  }

  try {
    cloudStatus = "جاري إنشاء رابط المشاركة...";
  ...
  created_by: authUser.id
    ...
    if (error) {
      cloudStatus = "فشل إنشاء الرابط: " + error.message;
      return;
    }

    const shareUrl = `${window.location.origin}/public-tree?token=${shareToken}`;

    try {
      await navigator.clipboard.writeText(shareUrl);
      cloudStatus = "تم إنشاء الرابط ونسخه ✓";
    } catch {
      cloudStatus = "تم إنشاء الرابط ✓";
      window.prompt("انسخ الرابط:", shareUrl);
    }
  } catch (err) {
    cloudStatus = "خطأ: " + String(err);
  }
}
  async function cloudSave() {
   ...
   cloudStatus = "تم الحفظ ✓ " + cloudProjectId;
  }
...
<div class="page" class:fullscreenMode={isWorkspaceFullscreen}>
    ...
    <div class="topbar">
      <div class="toolbarMain">
        <div class="row">
          ...
          <button class="btn" type="button" onclick={() => { showLineageDiagnostics = !showLineageDiagnostics; saveNow(); }}>
            {showLineageDiagnostics ? "🧪 إخفاء تشخيص النسب" : "🧪 إظهار تشخيص النسب"}</button>
 	  <button class="btn" onclick={createShareLink}>إنشاء رابط 🔗</button>
</div>
      ...
      {#if showLineageDiagnostics}
        ...
        <div class="toolbarCard diagCard">
          ...
          <div>الرقم: {lineageDiagnostics.selectedId ? `(${lineageDiagnostics.selectedId})` : ""}</div>
  ...
  "
>
  <span style="opacity: 0.7; font-weight: 600;">السلسلة:</span>
  <span style="margin-right: 6px;">
    {lineageDiagnostics.chainNames.join("  بن  ")}
  </span>
</div>
          {/if}
