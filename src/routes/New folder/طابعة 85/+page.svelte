<script>
   ...
   * - أي مشروع قديم بمساحة مختلفة يتم ترحيله إلى 85cm عند التحميل
   */
  ...
  const WORKSPACE_CM = 85;
  const WORKSPACE_MM = WORKSPACE_CM * 24.15; // 850mm
...
let printPaper = "SQ85";       // SQ85 | A4 | A3 | A2 | A1
let printOrientation = "portrait"; // portrait | landscape
let printErr = "";
...
function getPageSizeMm() {
  const p = PAPER_MM[printPaper] || PAPER_MM.SQ85 || PAPER_MM.A4;
  if (printPaper === "SQ85") return { w: p.w, h: p.h };
  if (printOrientation === "landscape") return { w: p.h, h: p.w };
  return { w: p.w, h: p.h };
}

function getPrintPaperLabel() {
  if (printPaper === "SQ85") return "85 × 85 سم";
  return printPaper;
}
...
function resetSelectedLinkBendsFromPanel() {
  ...
  // ويعالج ترحيل المشاريع القديمة من أي مساحة سابقة إلى 85cm.
  ...
  // =====================================================
  // --------------------------------
  // ترحيل المشاريع القديمة من أي مساحة محفوظة إلى مساحة 85cm
  // --------------------------------
  ...
  <div class="modalBack" onclick={closePrintModal}>
    <div class="modal" onclick={(e) => e.stopPropagation()}>
      ...
      <div style="opacity:.8; font-size:13px; line-height:1.5;">
        ...
        مساحة العمل محفوظة الآن على <b>85 × 85 سم</b>، ويمكنك فتح نافذة الطباعة ثم اختيار <b>Save as PDF</b> للحصول على ملف PDF بنفس المقاس.
      </div>
      ...
      <div class="grid" style="margin-top:12px;">
        ...
        <div>
          ...
          <select class="input" bind:value={printPaper}>
            <option value="SQ85">85 × 85 سم</option>
        ...
        <div>
          ...
          <select class="input" bind:value={printOrientation} disabled={printPaper === "SQ85"}>
            <option value="landscape">أفقي</option>
            <option value="portrait">عمودي</option>
          </select>
          {#if printPaper === "SQ85"}
            <div style="font-size:12px; opacity:.72; margin-top:6px;">في المقاس المربع الاتجاه ثابت تلقائياً.</div>
          {/if}
        </div>
      </div>
      ...
      <div style="margin-top:10px; font-size:13px; opacity:.78; line-height:1.6;">المقاس الحالي: <b>{getPrintPaperLabel()}</b>. لحفظه PDF اختر الطابعة <b>Save as PDF</b> من نافذة الطباعة.</div>

      <div class="actions">
        <button class="btn" type="button" onclick={closePrintModal}>إلغاء</button>
        <button class="btn primary" type="button" onclick={printMode === "poster" ? doPrintPoster : doPrintWorkspace}>🖨️ فتح نافذة الطباعة / PDF</button>
      </div>
    </div>
  </div>
{/if}
